    ...
    InterceptorsWrapper(
      onRequest: (options, handler) {
        ...
        if (sl<TokenInfo>().authToken != null) {
          options.headers['Authorization'] = 'Bearer ${sl<TokenInfo>().authToken!.accessToken}';
        }
...
Future<T> handleAsync<T>(Future<T> Function() onTry, {String unAuthorizedMsg = "Session Expired, please login"}) async {
  try {
    return await onTry();
  } on DioException catch (e) {
    if (e.response == null) throw Exception("Network Issue");

    if (e.response!.statusCode == 404) throw MyAppException(e.response!.data["Message"] ?? "not found");
    ...
    if (e.response!.statusCode == 400) throw MyAppException(e.response!.data["message"] ?? "invalid request");
